...
from src.deploy_model import load_model, model_card
...
from src.crm_integration import (
    score_customers,
    export_crm_csv,
    get_tier_summary,
    top_at_risk,
...
from src.ab_testing import run_ab_test, power_analysis_table
...
with st.sidebar:
    st.markdown(
        '<div style="font-size:2.8rem;margin-bottom:0.1rem">📡</div>',
        unsafe_allow_html=True,
    )
...
if page == "🏠  Today's Overview":
    ...
    st.caption(
        "These are your highest-risk customers — prioritise these for outreach today."
    )
    top5 = top_at_risk(scored, n=5)[
        ["churn_proba", "risk_tier", "retention_offer", "priority_rank"]
    ]
    top5 = top5.rename(
        columns={
...
# ── PAGE 2 — Customers to Contact ────────────────────────────────────────────
    ...
    for tier, label, offer, color in [
        (
            "High",
            "🔴 Urgent",
            "Personal phone call + 20% discount + contract upgrade offer",
            "#e74c3c",
        ),
        (
            "Medium",
            "🟡 Watch Closely",
            "Loyalty reward email + plan upgrade suggestion",
            "#f39c12",
        ),
        (
            "Low",
            "🟢 Safe",
            "Quarterly newsletter + standard satisfaction survey",
            "#2ecc71",
        ),
    ]:
...
# ── PAGE 3 — Why Is a Customer Leaving? ──────────────────────────────────────
    ...
    with st.spinner("Analysing customer patterns…"):
        _, shap_values, X_sample = compute_shap_values(
            model, X[feature_names], max_samples=max_samples
        )
    ...
    shap_sorted = _shap.Explanation(
        ...
        base_values=(
            shap_values.base_values[sorted_order]
    ...
    if proba >= 0.60:
        ...
        msg = (
            "This customer is very likely to cancel. Contact them as soon as possible."
        )
        action_html = """
        <div class="action-urgent">
    ...
    m1, m2 = st.columns(2)
    m1.metric(
        "Position in risk ranking",
        f"#{rank} of {n_customers}",
    ...
    m2.metric(
        ...
        (
            "Priority call"
            if proba >= 0.60
            else "Send offer email" if proba >= 0.36 else "Routine check-in"
        ),
    )
    ...
    with st.expander("📊 See detailed technical chart (for the data team)"):
        st.caption(
            "This chart shows exactly how much each factor shifts this customer's risk score."
        )
...
# ── PAGE 4 — Campaign Results ─────────────────────────────────────────────────
    ...
    power_df = power_analysis_table(base_rate=base_rate).rename(
        columns={
...
# ── PAGE 5 — System Health ────────────────────────────────────────────────────
    ...
    if drifted_feats:
        ...
        st.caption(
            "These customer attributes look different from when the system was last trained."
        )
    ...
    with st.expander("📊 See detailed technical data (for the data team)"):
        ...
        st.caption(
            "Change score: < 0.10 = Stable  |  0.10–0.25 = Monitor  |  > 0.25 = Needs refresh"
        )
